...
const TaskSection = memo(function TaskSection({
  ...
  return (
    <div className="border-l-2 border-outline-subtle dark:border-outline-subtle-dark ml-1 pl-2.5 py-1">
      {/* Task header */}
      <button
        onClick={onToggle}
        ...
        type="button"
      >
        <ChevronRight className={`w-3 h-3 text-content-tertiary dark:text-content-muted-dark transition-transform ${isExpanded ? 'rotate-90' : ''}`} strokeWidth={1.75} />
        <Bot className="w-3 h-3 text-content-secondary dark:text-content-secondary-dark" strokeWidth={1.75} />
        <span className="text-xs font-medium text-content-primary dark:text-content-primary-dark">{task.agent}</span>
        <span className="text-xs text-content-muted dark:text-content-muted-dark truncate flex-1">{task.task}</span>
        {isComplete ? (
          <CheckCircle className="w-3.5 h-3.5 text-emerald-500 shrink-0" strokeWidth={1.75} />
        ) : (
          <Loader2 className="w-3.5 h-3.5 text-amber-500 animate-spin shrink-0" strokeWidth={1.75} />
        )}
      </button>
      ...
      {!isExpanded && toolCount > 0 && (
        <div className="ml-5 text-[10px] text-content-muted dark:text-content-muted-dark font-mono">
  ...
  return (
    <div className="space-y-1">
      {summary.agents.map((agent, idx) => (
        <div key={idx} className="border-l-2 border-outline-subtle dark:border-outline-subtle-dark ml-1 pl-2.5 py-1">
          <div className="flex items-center gap-1.5">
            <Bot className="w-3 h-3 text-content-secondary dark:text-content-secondary-dark shrink-0" strokeWidth={1.75} />
            <span className="text-xs font-medium text-content-primary dark:text-content-primary-dark">{agent.agent}</span>
            {agent.model && (
              <span className="text-[10px] text-content-muted dark:text-content-muted-dark font-mono truncate">{agent.model}</span>
            )}
          </div>

          {agent.task && (
            <div className="ml-5 text-[10px] text-content-muted dark:text-content-muted-dark truncate" title={agent.task}>
              {agent.task}
            </div>
          )}
          ...
          {agent.toolBreakdown.length > 0 && (
            <div className="ml-5 mt-1 flex flex-wrap gap-1">
              {agent.toolBreakdown.map((b) => (
                <span
                  key={b.name}
                  className="text-[10px] font-mono px-1.5 py-0.5 bg-surface-subtle dark:bg-surface-dark-subtle border border-outline-subtle dark:border-outline-subtle-dark rounded text-content-secondary dark:text-content-secondary-dark"
                >
                  {b.name} ×{b.count}
                </span>
              ))}
          ...
          <div className="ml-5 mt-0.5 text-[10px] text-content-muted dark:text-content-muted-dark font-mono">
...
export const SubagentToolCard = memo(function SubagentToolCard({ 
  ...
  return (
    <div className="w-full border border-outline-default dark:border-outline-default-dark rounded-xl overflow-hidden bg-surface dark:bg-surface-dark my-1.5 shadow-xs transition-colors">
      ...
      >
        {/* Expand indicator */}
        <ChevronRight className={`w-3.5 h-3.5 text-content-tertiary dark:text-content-muted-dark transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`} strokeWidth={1.75} />
        ...
        <div className={`p-1 rounded-md border ${
          isError
            ...
            : isPending
            ? 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-900/40 text-amber-500'
            : 'bg-surface-subtle dark:bg-surface-dark-subtle border-outline-subtle dark:border-outline-subtle-dark text-content-secondary dark:text-content-secondary-dark'
        }`}>
        ...
        <div className="px-3.5 py-2.5 space-y-2.5">
          ...
          {effectiveMode && (
            <div className="text-xs text-content-muted dark:text-content-muted-dark">
              Mode: <span className="font-mono text-gray-700 dark:text-content-primary-dark">{effectiveMode}</span>
          ...
          {summary ? (
            ...
            <>
              ...
              {subagentData?.summary && (
                <div className="mt-2 p-2.5 bg-surface-subtle dark:bg-surface-dark-subtle rounded-lg border border-outline-subtle dark:border-outline-subtle-dark">
                  <div className="text-xs font-medium text-content-primary dark:text-content-primary-dark mb-1">Summary</div>
                  <div className="text-xs text-content-secondary dark:text-content-secondary-dark whitespace-pre-wrap">
                    {subagentData.summary}
                  </div>
                </div>
              )}
              ...
              {/* Stats footer */}
              <div className="flex items-center gap-3 text-[10px] text-content-muted dark:text-content-muted-dark pt-1.5 border-t border-outline-subtle dark:border-outline-subtle-dark font-mono">
                {totalToolCalls > 0 && (
          ...
          {result?.output && (
            <div className="pt-1">
              <button
                ...
                className="text-[10px] text-content-muted dark:text-content-muted-dark hover:text-content-primary dark:hover:text-content-primary-dark underline"
                type="button"
              >
              ...
              {showRawOutput && (
                <pre className="mt-1 p-2.5 bg-surface-subtle dark:bg-surface-dark-subtle rounded-lg border border-outline-subtle dark:border-outline-subtle-dark text-[11px] font-mono text-content-secondary dark:text-content-secondary-dark overflow-x-auto max-h-40 overflow-y-auto">
                  {result.output}
                </pre>
              )}
            </div>
      ...
      {!isExpanded && bgActive && (
        <div className="px-3.5 py-2 bg-surface-subtle/50 dark:bg-surface-dark-subtle/50 border-t border-outline-subtle dark:border-outline-subtle-dark">
          <div className="flex items-center gap-2 text-xs text-content-secondary dark:text-content-secondary-dark flex-wrap">
            {childModel && (
              <span className="font-mono text-[10px] text-content-muted dark:text-content-muted-dark truncate max-w-[55%]" title={childModel}>
                {childModel}
              </span>
            )}
            <span>{childStatus === 'completed' ? '✓ completed' : childStatus === 'running' ? 'running' : childStatus}</span>
            {childDuration && <span className="font-mono text-[10px] text-content-muted dark:text-content-muted-dark">{childDuration}</span>}
        ...
        <div className="px-3.5 py-2 bg-surface-subtle/50 dark:bg-surface-dark-subtle/50 border-t border-outline-subtle dark:border-outline-subtle-dark">
          <div className="flex items-center gap-2 text-xs text-content-secondary dark:text-content-secondary-dark flex-wrap">
            {summary.agents[0]?.model && (
              <span className="font-mono text-[10px] text-content-muted dark:text-content-muted-dark truncate max-w-[55%]" title={summary.agents[0].model}>
                {summary.agents[0].model}
              </span>
            )}
