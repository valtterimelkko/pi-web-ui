...
A useful practical corollary is that the eventual implementation may borrow lower-level mechanics from existing coding-agent memory systems — for example capture pipelines, retrieval strategies, audit trails, and replay surfaces — while still preserving the Agent OS's own continuity model. In other words, borrowed infrastructure is acceptable, but it should remain subordinate to the stronger memory structure of identity, role, project, thread, horizon, and evidence.
