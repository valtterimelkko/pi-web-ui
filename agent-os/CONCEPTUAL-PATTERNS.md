...
### Memory infrastructure vs memory ontology

A useful clarification is that the Agent OS may borrow substantial machinery from other memory systems without inheriting their root worldview.

Examples of implementation machinery that may be worth borrowing include:
- lifecycle-based raw capture
- hybrid lexical/semantic/graph retrieval
- bounded context assembly for warm starts
- consolidation, replay, audit, and provenance surfaces

However, those should be treated as **memory infrastructure**.
They should not by themselves determine the ontology of Agent OS memory.

So the governing distinction should remain:
- **memory ontology** = user, role, project, thread, horizon, evidence
- **memory infrastructure** = capture, indexing, retrieval, consolidation, replay, audit

This preserves a key rule:
- the Agent OS can borrow strong mechanics from coding-agent memory systems
- without collapsing into a session-centric, project-repo-centric, or capability-first identity
