...
The expected Internal API contract version is published by `GET /api/v1/health` and recorded with its full changelog in [`docs/INTERNAL-API-CONTRACT.md`](./docs/INTERNAL-API-CONTRACT.md) — that file is the version authority; do not maintain a second version summary here. In production, keep `INTERNAL_API_BLOCKED_PI_PROVIDERS` unset/empty (since 2026-09-11 the direct `openai` provider and the OpenRouter gateway catalogue are intentionally served through the Internal API) or set e.g. `openai,openrouter` to re-block either for cost protection; this does not affect browser model use, dictation/Drive Mode dictation, or TTS.
