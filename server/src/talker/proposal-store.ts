/**
 * Proposal lifecycle and the operator's composing draft (Voice Mode).
 *
 * This module holds both halves of the Proposal object:
 *
 *   1. THE PROPOSAL (Voice Mode execution plan, Phase 2 / Track A) — the
 *      four-object kernel's one live proposal per lane, carrying `{ id,
 *      version, sha256, original, tidied, presentedVariant, status }` and
 *      created ONLY through the three promotion routes (direct address,
 *      accepted offer, parked-item promotion). `ProposalStore` is the
 *      authority object; it preserves the delivered card identity gate
 *      (version + content hash), the staleness refusal and the
 *      original-variant gate in one place.
 *
 *   2. THE COMPOSING DRAFT (plan §4.2, P25) — the shipped cascade's
 *      accumulating verbatim record and confirmation-window machinery
 *      (`PendingProposalStore`, `UtteranceLog`, proposal descriptors). This
 *      moved here unchanged when Phase 2 introduced the four objects; the
 *      relay gate and its suites are pinned on exactly these bytes.
 *
 * The older name `pending-proposal.ts` is retained as a re-export shim for
 * import stability; new code should import this module.
 *
 * Original header (draft machinery):
 *
...
// ─────────────────────────────────────────────────────────────────────────────
// THE PROPOSAL OBJECT (Voice Mode execution plan, Phase 2 / Track A)
//
// One concrete thing for the worker: exact bytes, version and hash, both
// variants retained, at most one live proposal per lane (intent §16.3). A
// proposal is created ONLY by explicit promotion — direct address, acceptance
// of a validated talker offer, or promotion of a parked item (§18.1). It is
// never accumulated silently from conversation, and the thread has no route
// into it.
//
// The delivered card gate is preserved here verbatim:
//   - a confirmation echoes { version, sha256 } — the version alone would pass
//     a cancel-then-retype of the very same words, the hash alone would pass a
//     partial release that left different bytes on the card; both must match;
//   - a non-live proposal (released, cancelled, superseded) refuses;
//   - the 'original' variant exists only where the current proposal actually
//     advertised one (raw bytes different from the tidied relay text).
// ─────────────────────────────────────────────────────────────────────────────

/** The three — and only three — routes that create a proposal (intent §18.1). */
export type PromotionRoute = 'direct_address' | 'accepted_offer' | 'parked_item_promotion';

export const PROMOTION_ROUTES: readonly PromotionRoute[] = [
  'direct_address',
  'accepted_offer',
  'parked_item_promotion',
];

export type ProposalStatus = 'draft' | 'presented' | 'released' | 'cancelled' | 'superseded';

export interface Proposal {
  id: string;
  laneId: string;
  /** Store-wide creation counter; the identity half that moves with the bytes. */
  version: number;
  /** Content hash over the release bytes (see proposalHash). */
  sha256: string;
  /** The operator's raw bytes for an original-variant release (=== tidied when nothing was removed). */
  original: string;
  /** The relay bytes a default confirmation releases. */
  tidied: string;
  /** Which variant the card showed; null until presented. */
  presentedVariant: ReleaseVariant | null;
...
/** A route outside the three explicit promotion routes was attempted. */
export class UnknownPromotionRouteError extends Error {
  readonly code = 'unknown_promotion_route';

  constructor(route: string) {
    super(`Unknown promotion route '${route}': a proposal is created only by direct address, an accepted offer, or a parked-item promotion.`);
    this.name = 'UnknownPromotionRouteError';
  }
}
...
/**
 * The outcome of taking a proposal for release. Only 'taken' releases
 * anything; every other kind is a refusal with the current proposal attached
 * so the caller can quote what is really held.
 */
export type ProposalTakeResult =
  | { kind: 'taken'; proposal: Proposal }
  | { kind: 'not_found' }
  | { kind: 'not_live'; proposal: Proposal }
  | { kind: 'stale'; proposal: Proposal }
  | { kind: 'original_not_offered'; proposal: Proposal };
...
export class ProposalStore {
  ...
  /**
   * The only creation path. `route` is typed to the three promotion routes and
   * re-checked at runtime, so a thread turn (or any other object) cannot be
   * smuggled in as a proposal. One live proposal per lane: an explicit new
   * promotion supersedes the previous live one rather than accumulating.
   */
  create(input: {
    laneId: string;
    ...
    if (!PROMOTION_ROUTES.includes(input.route)) {
      throw new UnknownPromotionRouteError(String(input.route));
    }
    const live = this.live(input.laneId);
    if (live) {
      const storedLive = this.byId.get(live.id);
      if (storedLive) storedLive.status = 'superseded';
    }

    const tidied = input.tidied;
    const original = input.original ?? tidied;
    const hasOriginal = original !== tidied;
    const proposal: Proposal = {
      id: `prop-${this.nextId++}`,
      laneId: input.laneId,
      version: this.nextVersion++,
      sha256: proposalHash(tidied, hasOriginal ? original : undefined),
      original,
      tidied,
      presentedVariant: null,
      status: 'draft',
      promotionRoute: input.route,
      sourceUtteranceId: input.sourceUtteranceId,
      createdTurn: input.createdTurn,
      createdAt: input.createdAt ?? this.now(),
    };
    this.byId.set(proposal.id, proposal);
    return copyProposal(proposal);
  }

  get(proposalId: string): Proposal | null {
    const found = this.byId.get(proposalId);
    return found ? copyProposal(found) : null;
  }

  /** The single live proposal of a lane (draft or presented), or null. */
  live(laneId: string): Proposal | null {
    for (const proposal of this.byId.values()) {
      if (proposal.laneId === laneId && (proposal.status === 'draft' || proposal.status === 'presented')) {
        return copyProposal(proposal);
      }
    }
    return null;
  ...
  /** Every proposal ever created, oldest first, as copies. */
  list(): Proposal[] {
    return [...this.byId.values()].map(copyProposal);
  }

  /**
   * Record that the card was shown, and which variant it presented. Returns
   * null when the proposal is not live (nothing to present any more).
   */
  present(proposalId: string, variant: ReleaseVariant): Proposal | null {
    const stored = this.byId.get(proposalId);
    if (!stored) throw new ProposalNotFoundError(proposalId);
    if (stored.status !== 'draft' && stored.status !== 'presented') return null;
    stored.status = 'presented';
    stored.presentedVariant = variant;
    return copyProposal(stored);
  }

  /**
   * Validate a confirmation against the card identity and consume the
   * proposal atomically. The gates, in the delivered order: live → identity
   * (version AND sha256) → original variant advertised. Nothing is consumed by
   * a refusal.
   */
  takeForConfirmation(input: {
    proposalId: string;
    variant?: ReleaseVariant;
    identity: { version: number; sha256: string };
  }): ProposalTakeResult {
    const stored = this.byId.get(input.proposalId);
    if (!stored) return { kind: 'not_found' };
    if (stored.status !== 'draft' && stored.status !== 'presented') {
      return { kind: 'not_live', proposal: copyProposal(stored) };
    }
    if (stored.version !== input.identity.version || stored.sha256 !== input.identity.sha256) {
      return { kind: 'stale', proposal: copyProposal(stored) };
    }
    const variant = input.variant ?? 'tidied';
    if (variant === 'original' && stored.original === stored.tidied) {
      return { kind: 'original_not_offered', proposal: copyProposal(stored) };
    }
    stored.status = 'released';
    stored.presentedVariant = variant;
    return { kind: 'taken', proposal: copyProposal(stored) };
  }

  /** The operator abandoned a proposal; it can never release. */
  cancel(proposalId: string): Proposal {
    const stored = this.byId.get(proposalId);
    if (!stored) throw new ProposalNotFoundError(proposalId);
    if (stored.status === 'draft' || stored.status === 'presented') stored.status = 'cancelled';
    return copyProposal(stored);
  }
}
