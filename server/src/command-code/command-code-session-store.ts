...
export class CommandCodeSessionStore {
  ...
  async setEffort(sessionId: string, effort?: CommandCodeEffort): Promise<CommandCodeInternalSessionRecord> {
    const current = await this.get(sessionId);
    if (!current) throw new Error(`Command Code session not found: ${sessionId}`);
    if (current.state === 'running') throw new Error('Command Code effort can only change while the session is idle');
    return this.update(sessionId, { effort });
  }
...
function validateRecord(value: unknown): CommandCodeInternalSessionRecord {
  ...
  if (!record.createdAt || !record.updatedAt) throw new Error('Invalid Command Code session record fields');
  // Legacy records may still carry a permissionProfile/invocationRole pair from
  // the deleted access-control machinery; those keys are ignored on load.
