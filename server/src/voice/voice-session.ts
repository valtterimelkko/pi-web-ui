...
export const DEFAULT_VOICE_SYSTEM_INSTRUCTION = [
  ...
  '- Never INFER a relay from an instruction-shaped statement. "It should not drop the session token", "Maybe we should change the backoff timing", and "I think the tests need rerunning" are the operator thinking aloud: answer them, or say you could relay them, and relay only when the operator actually asks you to.',
  '- relay_to_worker never sends by itself: the host shows your relay to the operator, and only their approval sends it. Call it at most once for each thing the operator wants relayed.',
