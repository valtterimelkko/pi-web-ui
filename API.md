...
`GET /api/v1/health` and `GET /api/v1/capabilities` publish contract metadata (`pi-web-ui-internal-api`, `/api/v1`, current contract version — see `docs/INTERNAL-API-CONTRACT.md` as source of truth, currently `1.29.0`) for local consumers such as Agent OS. Contract `1.12.0` added source-owned durable/resident retention leases and resource-aware execution admission; `1.13.0` added truthful dispatch/approval/watchdog/session-identity semantics; `1.14.0` added payload-free run-liveness and bounded recovery evidence; `1.15.0` made disabled runtimes explicit; `1.16.0` added the Internal API-only Pi-provider execution policy; `1.17.0` added the Command Code runtime path; `1.18.0` added run-scoped Command Code usage evidence; `1.19.0` added normalized-output evidence on run receipts; `1.20.0` removed the Command Code invocation-role/attestation fields; `1.21` adds truthful Pi create responses (`model`/`modelSelector`, `422 MODEL_NOT_APPLIED`); `1.22` adds watch `onFire` parent-wake; `1.23` adds Command Code as a watch subject; `1.24` adds bounded `?mode=snapshot`/`?timeout` reads on `/sessions/:id/events`; `1.25` adds `MODEL_NOT_APPLIED` enforcement with candidate listing and liveness `workState`/`cessation`; `1.26` adds the copyable `selector` on `/models`, bare-id resolution (`fallbackApplied`/candidate listing), and long-poll `GET /watches/wait`; `1.27` adds the cross-runtime goal function (`GET/POST /goal`, `goal_state`/`goal_end`); `1.28` adds the `suggested` goal status (non-terminal `pendingSuggestion` awaiting owner approval); and `1.29` adds Claude SDK mid-turn `mode:steer` at the next tool boundary and model-aware `max` thinking. See `docs/INTERNAL-API-CONTRACT.md` (and `docs/RECENT-CHANGES.md`) for the authoritative changelog. Health also exposes per-runtime `runtimeHealth`, while diagnostics and `/sessions/:id/evidence` provide bounded troubleshooting evidence.
