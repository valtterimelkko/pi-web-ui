...
| 2026-09-24 10:41 | §15.4 | **F-8 EXECUTED (owner-approved in conversation): production restarted via the audited script** (`RESTART-REQUESTED` 10:41:05Z, drainage pre-flight passed, reason names the deploy + the owner approval). Verified after: service active (ActiveEnterTimestamp 10:41:24Z), `/api/v1/health` ok on contract **1.44.0**, 1690 sessions / **0 busy**, served bundle `index-CKNTqq6X.js` = the fresh build, and the new markers present in the compiled server (`provider_unresponsive` in voice-live-mount.js). Telegram: F-8 question delivery `sent`; the completion ping ("Internal API safe for the other agent to resume" + the D15-7 dogfooding go) delivery `sent` at 10:42:41Z. | `scripts/restart-pi-web-ui.sh` audit record; `/capabilities` read-back; notification `4937f0d1` |

| 2026-09-24 00:10 | §15.4 | **F-1 CLOSED: SOAK-10MIN-standard attempt-13 verifier PASS.** Full duration, 10 operator turns ≥ 8, exactly one reconnect; the acceptance line verbatim: *"soak pending-work survival verified: proposal prop-1 was pending at the reconnect and released, delivered and stored after it"*; the new relay produced prop-2 (presented → confirmed → delivered) and BOTH store probes matched. The chain that got there, one seam per attempt, each RED-first: `8fd7ddaa` (marker hygiene + ledger reset + D15-3 + C21 prompt), `3ee65713` (unresponsive-provider remint), `8fd993f6` (remint retires stale source candidates), `f704daf8` (stall 20 s > measured 12.8 s latency), `d38acf2d` (context-only replay + engagement anchor), `5283dbe9` (timebase-safe arming), `539e60b6` (mid-speech finals deferred, never dropped), `284f186d` (soak store probes per released identity). C18 natural wording + C11 amendment + C21 precondition all PASS live (21:15Z row). | `campaign/VERDICT.md` addendum; `SOAK-10MIN-standard/attempt-13`; `campaign/followup-summary.txt` |

| 2026-09-23 21:15 | §15.4 | **Follow-up live evidence (standard arm, master `8fd7ddaa`): C18 PASS, C21 PASS, C11 PASS.** C18 passes on the RESTORED natural confirm wording ("Yes, send the amended version.", fixtures re-synthesised WER 0). C21's reply now addresses the operator's version-check precondition explicitly (evaluator-style Q1 check recorded in the §15.4 addendum: pass — the campaign reply never mentioned the check). C11 — failed twice in the campaign — passes end-to-end: "Actually no, …" classifies statement, the model re-relays the full amended text, prop-2 replaces prop-1, confirm releases, delivered. | `campaign/followup-summary.txt`; `campaign/*-standard-followup.log`; `campaign/VERDICT.md` addendum |
| 2026-09-23 20:55 | §15.4 | **SOAK attempt-05 (post marker-hygiene + ledger-reset fixes, `8fd7ddaa`) FAILS on the same boundary — root cause now definitive.** After the reconnect the revived session's presentation read-back is barge-in interrupted (`serverContent.interrupted`), and from that moment the provider never produces model output again: input transcription continues, activity markers are matched, briefs are injected, yet no reply, no read-back, no relay_to_worker arrives for the rest of the run. Marker hygiene (unmatched activityEnd suppression) and the context-ledger reset are correct but insufficient — the wedge is INSIDE the provider session, and the records prove every fresh session responds immediately. Recovery implemented in `3ee65713`: the service detects a stalled model (operator statement/question with no model engagement for 12 s — confirm/cancel never arm it), remints the provider session once per wedge, marks `provider_unresponsive_remint` so the mount resets its context ledger, and replays the unanswered utterances as user turns (replays can never release: the gate still owns every send). | `SOAK-10MIN-standard/attempt-05`; `3ee65713` |
| 2026-09-23 20:40 | §15.4 | **The child-dispatch chain is abandoned with the owner's redirect to direct execution.** The adopted executor never executed (TURN_STALLED; session silent from 17:03:48Z), the fresh GLM conductor was never driven (parent truncated twice by its safety classifier), and the owner directed the follow-up to be executed directly without children. Two product commits land: `8fd7ddaa` (F-1 marker hygiene + ledger reset; F-2 D15-3 classifier + C18 corpus restore; F-3 C21 prompt) and `3ee65713` (the unresponsive-provider remint). | git master; plan §15 header (dispatch chain recorded) |
...
| 2026-09-23 17:00 | W4 | **CI GREEN on `6d6625db` — Gate G5's CI clause satisfied.** Three pre-existing reds fixed to get there, each diagnosed from the job log, never assumed: (1) the **changed-source warning ratchet** was failing repo-wide (352 warnings vs the 326 ceiling — red for many runs before this programme): 58 unused-import warnings cleared across 40 test files → **294** (32 under the ceiling), touched suites re-run (client 159, server 522); (2) CI's `test:coverage` failed on **~85 legacy DriveMode tests** with `nativeSurface.setAutoReadBackActive is not a function` — the shared `tests/helpers/nativeLaneStub.ts` had not followed the surface's H2/K growth; one method added → client suite **147 files / 1655 tests green**; (3) the local coverage run's single `claude-sdk-signature-pin` failure was verified **local-only** (the same test passed on CI, ✓ 940 ms) and not chased. Final CI: Application correctness **success**, Docs checks **success**. | `gh run 35892168709`; `0e8a52b1`, `6d6625db` |
| 2026-09-23 16:45 | W4 | **PROGRAMME COMPLETE — Gate G5 achieved.** Repository gates on the frozen revision: docs checks 0, typecheck 0, build 0, lint 0 errors, lab 709/709, voice/ws/talker/pi 1758/1758. Canonical index updated (`docs/VOICE-MODE-INDEX.md` → campaign + verdict, `0b43db25`). Agent OS capture submitted (2 pending candidates + raw evidence). Production not deployed (owner gate never lifted). Final verdict `campaign/VERDICT.md`: `NO_CANDIDATE_MEETS_TARGET`, rule-4 tie retains standard, failing boundary and smallest next experiment named. | `campaign/VERDICT.md`; `coordination/{EV,RV}/` |
| 2026-09-23 16:30 | W4 | **Reviewer reconciliation.** `coordination/RV/` verdict **supported-with-findings**: all 34 cell verdicts independently reproduced offline (34/34), 24,382 artefacts re-hashed with zero mismatches, arm identity + blinding checked. Three MATERIAL findings folded into the verdict rather than smoothed: (F1) holdout wording reached the implementer lineage via `holdout-overlay.test.ts` before the freeze — the ledger's earlier claim is corrected above; (F2) the C24 audible-ack bar was retired **after** C24 failed both arms (director `79c0daed`, verifier `5fc3f1bb`) because M's product fix stops the lane on a switch — C24-standard's pass exists only under the relaxed bar; both arms were graded under it and the rationale matches contract §3.2, but this row is the explicit post-hoc record the reviewer asked for; (F3) monetary spend is **unverifiable** (no metered usage in any record; the spend ledger was never kept) — the verdict now states wall-clock ≈7 h 47 m of the 8 h ceiling (~97%, inside but not "far inside") and makes no spend claim. Minor findings (corpusHash semantics, non-unique confirm wording, one unfinalised L4-era attempt, STATE staleness) are recorded in `review.md`. | `coordination/RV/review.md`; `campaign/VERDICT.md` |
