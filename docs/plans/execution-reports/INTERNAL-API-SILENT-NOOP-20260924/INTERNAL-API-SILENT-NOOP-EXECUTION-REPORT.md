...
## Round 3 (second owner review, 2026-09-24) — all PASS

| # | Finding → fix | Evidence |
|---|---|---|
| R3-1 | **Eviction leak (bug):** performRecovery re-subscribed the internal `multi-<path>` rehydration handle and never released it — cleanup/eviction require `subscribers.size === 0`, so every recovered session was pinned against unloading forever → recoverSession now releases the handle after re-attaching (load-act-release, same as the voice relay) and filters any pre-existing `multi-` leak from the captured set | Failing test FIRST: multi-session-manager-recovery.test.ts round-3 cases (recovered idle session with no real clients → `getSubscribers()` empty, eviction predicate satisfiable; pre-existing `multi-` leak healed while real clients stay re-attached). RED observed (leaked handle present), then GREEN |
| R3-2 | **Runbook, critical:** backups written inside `extensions/` would be loaded as DUPLICATE extensions (loader loads every subfolder with an index.ts, loader.js:560) → backups now go to `/root/.pi/agent/extension-backups/<timestamp>/`; added a post-copy containment check that no unexpected subfolder exists in `extensions/` before continuing | PHASE-8-RESTART-RUNBOOK.md §1 + §3 step 4 |
| R3-3 | **Runbook, critical:** restart without a clean build gate could deploy a stale `server/dist` → added the pre-restart build gate: record HEAD hash, require a clean tracked tree, `npm run build` must exit 0 (production serves server/dist), record the dist sha256; restart only if the build succeeds | PHASE-8-RESTART-RUNBOOK.md §2 |
| R3-4 | Minor: submitPrompt's error path leaked the turn-start waiter (rejected prompt left the parked waiter in the map) → cancelled in the catch | multi-session-manager-submit-honesty.test.ts still green (all submission paths); code restructure reviewed in commit |
| R3-5 | Minor: the browser view was stale after recovery (status-only resync) → the client now re-switches to the recovered current session on `session_recovered` (useWebSocket tap), which pulls a fresh history snapshot; live evidence from the round-2 WS run already showed the connection receiving events after recovery, and the replay now also refreshes the transcript | client/src/hooks/useWebSocket.ts + sessionStore session_recovered case |
