...
### Phase 1–2 excellence-loop refinements (Luna/gpt-5.5 critical re-review → EXCELLENT)

A re-review with the stricter excellence bar surfaced gaps the initial "proceed" reviews had deferred-and-never-closed. Luna drove four review rounds (NOT-YET → GOOD-BUT-INCOMPLETE → EXCELLENT); every finding was TDD RED→GREEN. Suite 2738 → **2764** passing / 0 failed.

**Phase 2 — admission made conservative + complete:**
- **Projected PID admission guard:** refuses P2/P3 with `pid_pressure` when `pids.current + (activeExecutionTurns+1)·reservedPidsPerTurn ≥ pids.max` — accounts for admitted-but-not-yet-realised turns (a race test proves turns admitted while `pids.current` is unchanged still block), uses `≥` so projected use cannot land exactly on the ceiling. Control bypasses. Config `INTERNAL_API_ADMISSION_RESERVED_PIDS_PER_TURN` (256).
- **Projected host-memory gate:** the service cgroup bounds only this process; tmux/external work is outside it. New `host-pressure.ts` reads `/proc/meminfo` + PSI; admission refuses with `host_memory_pressure` when projected `MemAvailable − (activeExecutionTurns+1)·reserved/turn < hostMinimumHeadroomBytes`. `hostPressure` is `undefined` (not `false`) when telemetry is unreadable; `/capacity` exposes `telemetryAvailable`. Config `INTERNAL_API_ADMISSION_HOST_MIN_HEADROOM_MB` (512).
- **memory.high gate + memory.events + PSI truth:** reaching the soft `memory.high` boundary is now pressure (makes the surfaced truth actionable); `readServiceMemoryEvents` (oom/oom_kill/high) + PSI summaries surfaced in `/capacity` as observability.
- **Batch-prompt admission durability (correctness bug):** `handleBatchPrompt` acquired admission *inside* `executePromptWithReceipt`, after `markStarted` — a refusal left the receipt "started" + generic `RUNTIME_ERROR`. Admission is now acquired *before* `markStarted`; `AdmissionCapacityError` → `rejectBeforeDispatch` + result `ADMISSION_CAPACITY_EXHAUSTED` + reason + retryAfter. Test added.
- **503 vs 429:** pressure refusals (memory/pid/host) → 503 service-unavailable; capacity refusals (global/runtime limit) → 429. Matches the claimed contract.
- **Single-read snapshot:** `evaluatePressure()` reads memory/pids/host ONCE each per evaluation (was 2× — could produce inconsistent snapshots under pressure).
- **Conservative production fallback:** in `NODE_ENV=production`, any unset safety knob falls back to conservative defaults (6/1/1536/768/256/512 MiB/PIDs) — a missing/mis-loaded `.env.production` cannot make the server run CPU-derived admission. Per-knob explicitness is logged at boot and surfaced as `admissionConfig` in `/capacity`. `.env.example` documents all knobs.

**Phase 1 — disabled-OpenCode contract made complete (no-spawn + fail-closed + stable errors):**
- **No-spawn violation fixed:** `/api/v1/models/refresh` → `refreshModels` → `warmModelCache` → `execFile('opencode')` spawned a real child while disabled; guarded at the route **and** defended inside `OpenCodeService.refreshModels`.
- **Every mutating/spawning entry point now fail-closes** with a contracted stable error: create (single `RUNTIME_UNAVAILABLE`, batch `RuntimeOpError`, WS `OPENCODE_UNAVAILABLE`), transfer (both paths), prompt (single/batch/WS), `set_model` + `set_thinking_level` (Internal API + WS), models list (Internal API + legacy web `/api/models`), models refresh, and `goal_control` pause/clear/resume (which previously mutated goal state — `resumeGoal` flips to "running" — with no guard). `doStart` remains defence-in-depth.
- **Honest scope:** read-only/metadata calls (availability probe, pin/unpin, status, replay/goal-event reads, permission resolution, shutdown/abort) intentionally remain allowed on historical sessions; the contract is "disabled OpenCode cannot start new work, spawn, refresh/warm models, or mutate model/thinking/goal execution state" — not "no state can ever change" (deliberate abort/delete/pin/cleanup still act).

**Out of Phase 1–2 (accepted, deferred):** ancestor-cgroup fallback walk (Phase 6 per-session cgroups); PSI-avg-threshold admission (host-mem gate covers the host-exhausted case; PSI surfaced as observability); `memory.events` delta tracking (cumulative counters surfaced); Phase 4 assignment state-machine/epoch; Phase 6 cgroup isolation.

**Luna (gpt-5.5) critical verdict: EXCELLENT.** After four review rounds Luna confirmed the disabled-runtime contract is now complete across every operational entry point and Phase 2 admission meets the excellence bar (projected PID/host gates, memory.high, 503/429, batch durability, single-read snapshots, conservative prod fallback + `/capacity` explicitness), with a precise scope caveat (read-only/control ops intentionally remain allowed).
...
### Excellence-loop refinements (Luna/gpt-5.5 critical review)

Phase 3 was driven through an execute → self-critique → Luna-critical-review loop until the verdict reached EXCELLENT (or the only remaining items were explicitly out-of-Phase-3). Each refinement is TDD RED→GREEN; suite grew 2709 → **2738** passing / 0 failed.

- **Drain fence (3.2 centerpiece, commit eb57ae3):** `RunReceiptManager` now DEFERS admission-lease release on cancel/stall until the runtime confirms cessation (`isRuntimeQuiescent`: claude/opencode/antigravity `isRunning` + Pi `status !== 'busy'`) or a bounded 30 s drain timeout. Normal completion (agent_end) releases immediately (cessation confirmed). Closes the "admission released before runtime cessation" hole Luna flagged — no false capacity release mid-drain.
- **Quarantine as capacity-debt (commit 5058aab):** on drain timeout the slot is NOT released (cessation still unconfirmed) — it is held as quarantined capacity-debt (`getQuarantinedCount`, surfaced as `quarantinedRuns` in `/capacity`) until restart/operator recovery. So the timeout never produces a false capacity release. Cross-run leakage is structurally impossible (leases are keyed by `runId`); a test proves a late `agent_end` for an already-cancelled run does not release twice.
- **Broker replay byte bound (commit eb61ceb):** the per-session replay buffer was count-bounded only (100); a stream of large events could grow memory within that count. Now also bounded by `replayBufferMaxBytes` (default 8 MiB); oldest trimmed on either cap; `clear`/`clearAll` drop byte totals.
- **Per-session disposal registry (commit 89fb035):** `SessionDisposalRegistry` — every per-session resource (queued-run correlations, observers, timers, snapshots, drain handles) registers a dispose handle; `handleDeleteSession` tombstones the session + runs all handles early (before runtime abort); `shutdown` runs `disposeAll`; idempotent; late registration after disposal runs immediately and cannot repopulate state. `/capacity` reports per-session remaining owner counts (`disposalOwners`). First real surface wired: Pi queued-run correlations + direct-dispatch tokens, which `handleDeleteSession` previously did not clean (a genuine leak).
- **Event-broker deletion fence (commit 68e1e0c):** the tombstone blocked late *registry registration* but not late *broker.publish* — a late runtime event after delete could recreate the replay buffer. `InternalApiEventBroker` now accepts an `isSessionDisposed` predicate; `publish()`/`subscribe()` drop calls for disposed keys. Routes wire the predicate to the disposal registry and tombstone the broker-key alias (Pi publishes under `entry.path`). The long-lived Pi/OpenCode broker-feeding observers are now disposal-owned (not just manually removed). Route-level regression test: delete → late `broker.publish` leaves no buffer/subscribers.
- **`/events` SSE stream disposal ownership (commit 4f9e0f7):** the live `/sessions/:id/events` SSE stream + its 15 s heartbeat were not disposal-owned — `broker.clear` can't close the HTTP response (broker holds no req/res), so a deleted session left an open connection + heartbeat until client disconnect. `handleSessionEvents` now registers a disposal handle that unsubscribes + `sse.complete()` (res.end + clearInterval) on delete; normal client disconnect unregisters the handle. Regression test: delete closes an active stream, clears ownership, and resolves the handler.
- **Disposal-registry reentrancy hardening (commit 6c3b444):** `dispose()` iterated the *live* owners array, so a handle that called its own `unregister()` (splice) mid-disposal could skip later handles. It now snapshots the array + deletes the map entry *before* iterating, so mid-disposal unregister is a harmless no-op. Regression test: a self-unregistering handle does not skip the next handle.
...
### Pause 4 decision
`proceed` — accurately scoped (per Luna/gpt-5.6-luna critical review): the arbiter **caps P2/P3 execution capacity** below total active-turn capacity (reserving slack for P0/P1 paths), **bounds control concurrency** via a separate control lane, and **emergency mode** preserves control under memory pressure. This protects against admission-slot starvation + unbounded control fan-in; it does **not** by itself protect control from shared-resource contention (event loop/memory/sockets) caused by in-flight P2 turns — that requires process isolation (Phase 6).

**Excellence pass (iterate→critical-evaluate, then Luna):**
- **Bounded control lane** (`server/src/internal-api/control-lane.ts`): P0/P1 control handlers run through a concurrency cap + queue-with-timeout (not unbounded bypass), so a control flood cannot monopolise shared resources. Applied to cancel/evidence/run-receipt/control/approval/delete/history; stats exposed in `/capacity` (`control.inFlight`/`queued`). Route test proves a real handler acquires+releases it.
