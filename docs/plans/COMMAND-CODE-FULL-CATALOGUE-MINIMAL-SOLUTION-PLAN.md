...
>
> **Required consumer mirror:** Agent OS (`/root/agent-os`) where the Internal API contract/types are consumed
>
...
>
> **Target product catalogue:** the exact intersection of fresh CLI discovery, the reviewed GOAT-entitlement snapshot, and valid live capability evidence; at the planning baseline this is 35 models
>
...
- make browser, REST, WebSocket, Internal API, replay, evidence, and Agent OS consumer projections agree;
...
1. **Internal API model cohort:** replace the Qwen/Muse-only attested shadow execution cohort with every freshly discovered, capability-verified model included in the operator's GOAT plan.
...
Historical Agent OS memory remains historical evidence. This plan does not edit or approve compiled memory. The operator's new authority and the shipped repository after implementation become the current implementation authority.
...
| LD6 | All effective GOAT-eligible models must be executable through both the browser path and the attested Internal API path, subject only to truthful temporary health/capacity/credit/rate-limit conditions—not a repository hard-coded pair. |
...
| LD17 | A provider-free fixture is necessary but insufficient. At least one blocking done-observation must use the real installed `cmd` binary, real authentication, real GOAT entitlement, and a real provider response. |
...
| LD20 | Completed owned changes are committed and pushed on the current branch without creating a new branch, but only after all blocking gates pass and a separate execution has been authorised. |
...
The official GOAT page states **35/54 models** at the planning date. The model registry can change; implementation must record source URL, retrieval time, hash, reviewed IDs, and drift rather than treating “35” as eternally true.

### 4.2 GOAT-eligible baseline: 35 models

These are selectable/executable at the planning baseline:
...
2. Keep raw CLI discovery separate from effective product eligibility.
3. Compute:

   ```text
   effective = rawDiscovered ∩ reviewedGoatEligible ∩ capabilityKnown
...
4. `effective` is the only selectable/executable model set.
5. `excludedPremium`, `missingEligible`, and `unreviewed` are diagnostics, not selectable model entries.
6. If the official GOAT page and installed CLI disagree, preserve bounded evidence, keep uncertain IDs excluded, and ask the operator before revising the reviewed snapshot.
7. Do not scrape plan pages on every server startup. Use a deterministic refresh/audit command with reviewable output and explicit update workflow.
8. Do not send test prompts to determine premium eligibility; that could consume top-up credits or trigger upgrade/payment paths.
...
| Model rejected after selection | running backend retained pair-only browser execution | every effective GOAT model passes browser policy |
...
| Internal API pair restriction | route/service/config/attestation hard-code Qwen/Muse | every effective GOAT model can create/prompt via attested Internal API |
| Raw catalogue conflates availability and entitlement | 54 global IDs were treated as target product catalogue | 35-model GOAT effective catalogue with 19 excluded at baseline |
...
- Full browser and Internal API usability for the effective catalogue.
...
| B-API-4 | Contract | Contract version is bumped appropriately; Pi Web UI and Agent OS types, validators and mirrored docs agree | failing contract/mirror tests in both repos |
...
9. Update protocol/contracts and Agent OS mirror.
...
- Replace the monolithic “full executable catalogue” concept with:
  ...
  - effective executable catalogue;
...
### WP2 — Generalise execution policy from two models to the effective GOAT set
...
- Replace `assertCommandCodeModel()` pair semantics with clear domain functions such as:
  ...
  - raw discovered ID;
  - effective GOAT-eligible executable ID;
...
- Ensure Agent OS routing policy remains separate from technical model availability; making a model executable does not make it an Agent OS finalist or default.
...
- Make `/models` distinguish effective executable catalogue from diagnostic drift metadata.
...
- Synchronise Agent OS client/types/tests/mirrored contract in the same execution series.
...
- relevant Agent OS mirrored contract/docs.
...
1. assert raw 54 and effective 35 policy projections;
2. assert all 19 excluded IDs are absent from selectable routes;
3. select a non-pair effective model in the actual browser UI;
...
4. verify catalogue contains the exact effective GOAT set and excludes all 19 premium IDs;
...
| DO2 | The model picker contains exactly the current reviewed GOAT-effective catalogue—35 at the baseline—not all 54 raw models. |
...
| DO11 | Internal API `/models` exposes all and only effective GOAT models as executable. |
...
# Agent OS focused/full contract/client tests when its mirror changes
git diff --check
git status --short
git diff --stat
