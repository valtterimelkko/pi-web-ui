...
> **Correction (Internal API contract 1.13.0):** the Phase H causal diagnosis above was inverted. Turn 1 had already completed; idle Pi `follow_up()` only queued into a live agent loop that no longer existed, so the accepted message was silently dropped. Contract 1.13.0 state-checks the mode and visibly promotes idle `follow_up` to a real `prompt` turn.
