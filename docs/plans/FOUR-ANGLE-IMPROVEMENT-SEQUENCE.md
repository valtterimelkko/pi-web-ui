...
## Owner-authorised catalogue extension — 8 September 2026

During baseline execution, the owner confirmed the GOAT subscription excludes premium models and authorised repair of the failed weekly Command Code refresh, including its brittle fixed-size test, catalogue completeness and canonical documentation with official provider references. Preserve the two newly confirmed exclusions (`claude-fable-5-1`, `gpt-6-astra`); the previously unowned generated catalogue changes are now within this bounded repair's scope.

The owner additionally requested inspection and autonomous inclusion of Monday refresh for Command Code models consumed through **Pi CLI, Pi SDK, Internal API Pi runtime and the frontend**, noting Pi is the primary harness used. Inspection supports inclusion: the existing native Pi provider in `/root/pi-enhancement/commandcode-provider/` has a generated catalogue shared by those surfaces, but refresh requires canonical case-sensitive IDs and independently verified route/thinking/vision evidence. Canonical completed integration record: that directory's `PLAN.md`; generator: `scripts/generate-commandcode-models.mjs` in the sibling repository.

Bounded addition to Steps 1/3/10, not a new provider implementation: repair native catalogue integrity, then extend existing weekly orchestration with delta-based, bounded Pi-provider refresh, explicit pending/inconclusive status, last-known-good preservation and separate generation/activation reporting. Reuse the existing generator and probe owners; no four-way catalogue duplication, no silent expansion of claimed thinking/image capabilities, no blanket full-catalogue billed sweep. Establish TDD for discovery/delta/retry/generation failure and budget limits; validate canonical IDs and capabilities through the four real disposable consumer surfaces, with targeted provider-backed probes only within explicit authority/headroom. Coordinate sibling paths before editing. Live extension activation and production restart remain end-of-task permission gates. New tests must reject incomplete/stale refresh evidence and show transient probe failures neither remove known-good models nor disappear as completed work. Record measured scope, remaining unverified additions, cleanup and quota boundaries in the execution report.
