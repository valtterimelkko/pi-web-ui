...
- **Contracted:** `GET /health` and `GET /capabilities` publish contract metadata (`pi-web-ui-internal-api`, `/api/v1`, the current contract version) so local consumers can detect the API surface they are using. See [`INTERNAL-API-CONTRACT.md`](./INTERNAL-API-CONTRACT.md) for the current version and its history.
...
**Pi model bindings are durable across rehydration (contract 1.33.0, 2026-09-03).** The create-time binding (model + thinking level) is persisted in the session registry, and control `set_model` / `set_thinking_level` update that stored binding (thinking level stores the clamped read-back value). When a dispatch finds the live session drifted from the stored binding — the Pi SDK restores history bindings only for sessions that already carry messages, so a session unloaded between create and dispatch (capacity eviction, server restart) rehydrates on the runtime default — the dispatcher re-applies the stored binding before the turn runs, publishes a `model_rebound` broker event (`data`: `intended`, `served`, `thinkingLevel`), and records the outcome on the run receipt: `servedModel` is the model actually bound at dispatch and `modelRebound` is `true` when a re-apply happened. A stored binding that can no longer be applied never falls back silently: an unresolvable model fails the run with `MODEL_NOT_APPLIED`, and a provider blocked by `INTERNAL_API_BLOCKED_PI_PROVIDERS` fails with `PROVIDER_NOT_ALLOWED` before the turn starts. Sessions created without a model keep the previous behaviour (the runtime default is their intended binding).
...
> session can be the observed child: its turns reach the watch from every
> source (Internal API dispatch, browser, follow-ups) via a service-level
> observer attached at registration, and the subject is pinned through the
> Command Code service. Command Code sessions are equally valid as
> `onFire` wake targets (parents).
...
### Pi goal truth (contract 1.45.0)

On Pi the API verifies at the command boundary that a goal action actually applied (the
extension persists synchronously before its command returns). A command that ran but did
not apply — the classic shape: a fenced goal-engine refuses mutation with only a warning
toast — answers **`409 GOAL_ACTION_NOT_APPLIED`** with `observedGoal`, `extensionWarnings`
(the extension's own notify text) and a `failed` receipt. This is a deliberate behaviour
change: before 1.45.0 the same call answered `200 accepted:true` with a `completed`
receipt. Honest no-ops answer `200` with `applied:false` plus a `reason`
(`already_inactive` for clear on an inactive/achieved goal, `already_paused`, `not_paused`);
a verified start carries `applied:true`. Busy-session queued commands keep the 1.27.0
accepted shape, because the goal state may legitimately lag the queued command mid-run.
