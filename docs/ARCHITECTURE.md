...
5. **Command Code** (`cmd -p`, one direct subprocess per session)
...
- Command Code native session IDs
...
### Command Code
- eligible models are what the CLI advertises minus a committed 19-model premium exclusion list (denylist, fails open); per-model effort selectors come from a committed table regenerated by `npm run commandcode:refresh-models`
- one direct `cmd` subprocess per session with ordinary host networking, a single `COMMAND_CODE_ENABLED` gate, CWD validated against `COMMAND_CODE_ALLOWED_CWD_ROOTS`, and a private per-session native home
- private normalized journals are the replay source; stderr, credentials, and native paths remain private
...
- `commandcode_available` (single enable gate)
- `/api/health/ready`
