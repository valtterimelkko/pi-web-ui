...
Read the advertised contract version and capability-gate optional behavior instead of assuming that every installation has the newest endpoint set. For contract `1.14.0` orchestration, preflight `/capacity`, but still handle prompt-time `429 ADMISSION_CAPACITY_EXHAUSTED` and its `Retry-After` header. Capability-gate `runLivenessEvidence`/`sessionRecoveryEvidence`; a terminal receipt and adapter residency do not by themselves prove nested-process or workspace quiescence.
