...
| Rate limits | Live limits are **concurrent sessions per project + TPM per usage tier**; free tier reportedly 3 concurrent sessions. **Operator-supplied 2026-09-16, pending L1 confirmation: Prepaid Tier 1 carries 2,000,000 TPM for both `gemini-3.8-live` and the Extended Thinking variant**, which supersedes the earlier "paid-tier TPM is not published" note. Standard Live counts only audio-round-trip tokens; **Extended Thinking also counts its background thinking tokens against TPM and the prepaid balance.** Tiers: Free / T1 (billing linked) / T2 ($100 + 3 days) / T3 ($1,000 + 30 days). | See §12.1: at ~25 audio tokens/s per direction a session uses roughly 0.15 % of a 2 M TPM budget, so **TPM is not the binding constraint — concurrency and the prepaid balance are**. Decision i. Preflight records the tier; a 429 or `RESOURCE_EXHAUSTED` inside a run = `environment` failure, budget-stop, never a quality score. Sequential runs. |
...
- **One minute of allowance is 33,333 tokens/s at 2 M TPM.** A live conversation uses ~50 tokens/s of it: about **0.15 %**. A full 15-minute session is on the order of 45,000 tokens — roughly two seconds' worth of the allowance. **Throughput is therefore not the binding constraint**, and the §24 risk that quota is tighter than expected is now largely answered: what remains is the **concurrent-session cap** and the **prepaid balance**, and decision i already serialises runs to one Live session.
