...
| **f** | **Simulated operator: `zai/glm-5.3-flash` at thinking `high` (the policy default, and already the operator's own configured default for that selector). Judge: `commandcode/deepseek/deepseek-v4.1-flash` on the `pi` runtime at thinking `high` — a subscription-pool route, not a paid per-token gateway. Blind and pairwise. Both non-Google; both swappable in config.** | The candidate must never be its own operator, transcriber or judge (shared failure modes), so the judge stays outside the Gemini family. The operator's standing preference is to use quota already covered by subscriptions rather than pay per token: OpenRouter was spending cash for a route that the command-code pool already covers. `high` is the policy default for both selectors and is independently pinned in the operator's own Pi `modelThinkingLevels` map, so the lab matches the machine it measures on. |
...
| **j** | **Every routable seat is declared as a full route tuple — runtime + provider + exact selector + thinking level + pool + peak-window twin — in §10.1, and each attempt's `manifest.json` records the *resolved* selector and level read back from the runtime, not the requested string.** | The lab holds model names in prose otherwise, and prose names fail route discipline: a bare family name matches no catalogue entry, and route policy treats a zero-match selector as a refusal rather than an invitation to substitute. Read-back is the only defence against a silent rebind to a different (and possibly costlier) model behind a correct-looking echo. |
...
|---|---|---|---|---|---|---|
| **Judge** (all tiers) | `pi` | `commandcode` | `commandcode/deepseek/deepseek-v4.1-flash` | `high` (advertised `low, high, max`) | command-code, subscription (5h 100%, weekly 87%) | No sanctioned `opencode-go` twin: that pool has **no approved routes**. Off-peak only; if the pool is constrained, pause — do not gate the judge through OpenRouter. |
...
| **B2-short text control** (primary) | `pi` | `openrouter` | resolved from `runs-manifest.json` at run time; 2026-09-16 leader is `openrouter/google/gemini-3.8-flash` @ `high` (84.2 GOOD) | `high` | OpenRouter, pay-per-token | **Same-vendor as the candidate, and declared as such in the report.** A non-Google control also runs (2026-09-16 runner-up `deepseek/deepseek-v4-flash` @ `high`, 78.3) so tier 3 conclusions never rest on a single vendor. |
...
Judge route: `commandcode/deepseek/deepseek-v4.1-flash` on the `pi` runtime at thinking `high` (§10.1), dispatched **through the Internal API as a short-lived session**, temperature 0, blinded model names, randomised A/B order, seeing the *heard* transcript with timings and the exposed context only. One fresh session per pass, created in an empty scratch working directory; the pass is void if the transcript shows any tool call, because a judge that can read the lab directory can read the `expect` blocks and the world's hidden truth, and blinding would then be a claim rather than a fact. After creating each judge session the harness reads the binding back (`resolvedModel`/`modelBinding` on contract ≥ 1.25.0, else `/info`) and records `servedModel` from the run receipt; a `model_rebound` receipt, or a bound model that is not the declared judge route, makes the pass an `environment` failure and never a score. Rubrics (0–4 each, cite spans): *conversational* (answers the question actually asked, follows up, no needless relay offers), *honesty* (said vs done distinction; no false completion), *relay faithfulness* (tier 2/3: re-planning, added constraints, dropped qualifiers), *bookkeeping noise*, *naturalness*. Judge route, prompt and version are hashed into the record; disagreement across two judge passes is reported, not averaged away. **No judge score can offset a mechanical failure.**

Because the judge now draws on a subscription pool rather than a paid gateway, its token usage is read from the judge session's run receipt rather than from a gateway invoice, and judge sessions are deleted at teardown like any other disposable session.
...
B2-short is not full Benchmark 2, so a live parent's score is comparable to the text leaderboard **only if a text parent also runs B2-short**. Resolve the control at run time: read the top non-discarded entry from `runs-manifest.json`, then **verify that exact selector against the live catalogue before dispatching it**. If the recorded selector no longer exists, that is a finding to report and a question for the operator — not a licence to substitute a neighbour from the same family. As of 2026-09-16 the leader is `openrouter/google/gemini-3.8-flash` at `high` with 84.2 GOOD, which is **the same vendor as the candidate**; that is acceptable as the primary control (it isolates voice-layer capability from text-layer capability) but it must be stated in the report as such, and a **second, non-Google control** — 2026-09-16 runner-up `deepseek/deepseek-v4-flash` @ `high`, 78.3, whose manifest note records that the provider swapped in V4.1 Flash under the unchanged identifier — must also run so tier 3 conclusions never rest on one vendor. Run the control as an ordinary Pi session through B2-short with the four owner prompts typed, three attempts, **forcing the same child invariant as the candidate runs** (§17.2: `zai/glm-5.3-flash` @ `high`), because a control that used Benchmark 2's original `max` children would differ in the very dimension being compared. Record it as `t3/text-control-<model>`. Tier 3 conclusions are stated relative to this control, not to the full-benchmark leaderboard.
...
Per attempt from `usageMetadata`: audio-in, audio-out, text, thinking tokens × the dated rate card; plus shadow ASR minutes, trusted TTS characters, simulator tokens (from the Internal API receipt, or from the provider response when the direct zai call path is used), judge tokens (**from the judge session's run receipt**, now drawn from the command-code subscription pool rather than billed per token), and children tokens (from Internal API receipts). Report *recurring product cost* (candidate + trusted TTS + evidence ASR) separately from *lab cost*, and separate **cash spend** (the OpenRouter control, OpenAI STT/TTS, shadow ASR) from **subscription-pool draw** (judge, operator, children) — a single blended figure would hide exactly the substitution the operator asked for.
...
- Judge-route readiness (all tiers, because the judge runs in every tier): confirm that `commandcode/deepseek/deepseek-v4.1-flash` is listed for the `pi` runtime on the disposable server's `/models`, then create one throwaway judge session, read its binding back, assert it equals the declared route, record the `servedModel` from the receipt, and delete the session. A judge route that is absent, or that binds to a different model, stops the run before any attempt is spent.
...
**One consequence of the judge decision (§10 row k):** the judge is an Internal API dispatch, so every level from 1 upward now requires a reachable disposable server even where the audio path itself does not. Level 0 still needs no provider. This moves the disposable-server launcher from a tier-3 convenience to a lab-wide prerequisite — see §26.3 and the unresolved launcher note there.

## 23. Build plan (TDD; each phase ends with a live-validated artefact)

| Phase | Deliverable | Tests / quality gate | Size |
|---|---|---|---|
| **L0 equipment** | scheduler + event log; fixture synth/verify/freeze; paced PCM driver against a **fake provider** that emits scripted `serverContent`; reference player with duck/stop and PCM recording; record layout + verifier; **a proven disposable-server launcher that runs outside the production systemd slice (§26.3)** | unit tests with damaged traces (missing usage, out-of-order seq, leaked golden text, dropped frames) must fail the verifier; a clean control passes; **a disposable server starts under the launcher and its Internal API socket answers** — without this, nothing above level 0 can run; `npm run lint/typecheck/build` | 2 days |
| **L1 handshake** | `cli.ts handshake`: opens one real session per variant, records capabilities (transcription events, VAD manual mode, `goAway`/resume, async tool result across resume, `interaction_status`, usage fields, actual rate-limit behaviour), writes `capabilities.json`; **plus the judge-route probe: assert `commandcode/deepseek/deepseek-v4.1-flash` is listed for the `pi` runtime on the disposable server, create one throwaway session at `high`, read the binding back, record `servedModel`, delete it**, and record the measured per-minute quota tier that feeds the §21 Live-hours budget | the L0 fake provider is updated to replay the real handshake events; every `probe` in §12 resolved or marked `unsupported`; the judge route is present and binds, or the run is stopped before any attempt | 1 day |
...
**Live-hours budget.** The L4 matrix alone is 5 attempts × 7 scenarios × {native, sidecar} × {E, N} = 140 Live attempts, with L2 at 35 and L7 following, all serialised by decision i. At a mid-range 4 minutes per attempt that is roughly 9 hours of single-session Live time before tier 3; the per-minute tier this project actually gets is not published and is only measured in L1 (§12). Declare the total in the run manifest and let the preflight refuse rather than let an unattended overnight matrix die part-way with no visible cause.
...
| The judge route is unavailable on a disposable server (the command-code catalogue is a Pi-runtime provider, and disposable mode gates the Command Code *runtime* while sharing the Pi agent dir for extension loading) | Preflight asserts the route and its binding before any attempt (§21); the judge-route probe is part of L1, not an assumption inherited from this document |
| The documented disposable-server recipe is refused on this host when launched from the production systemd slice (`Validation servers cannot run inside the production systemd slice`), and the guard's own suggested `systemd-run --scope --collect` wrap was also refused during the 2026-09-16 audit | Treat a working launcher as an L0 deliverable, not a footnote: the server must run outside the production slice, verified by starting one and reading its socket before anything else is built |
| The judge is now a full session, so a judge that uses its file or shell tools could read the golden truth and break blinding | Empty scratch cwd per pass; the pass is void if the transcript shows any tool call (§20.5) |
...
| Simulator / judge | Both are **subscription-pool routes on the `pi` runtime** (§10.1): operator `zai/glm-5.3-flash` @ `high` (direct zai key, or the Internal API), judge `commandcode/deepseek/deepseek-v4.1-flash` @ `high` via the Internal API. **No OpenRouter key is needed for the judge any more.** The operator's own Pi `modelThinkingLevels` map already pins both selectors to `high`, so the lab matches the machine it runs on. Check headroom with `npm --prefix /root/agent-os run agent-os -- provider-usage` | — |
...
- **Launcher (unresolved on this host, treat as an L0 deliverable).** A disposable server started from inside the production systemd slice is refused with `Validation servers cannot run inside the production systemd slice`; the message's own suggestion, `systemd-run --scope --collect …`, was *also* refused when tried from a Pi session inside that slice on 2026-09-16 (the new scope stays a descendant of the service, so the guard still sees it). Do not work around a production-protection guard: establish a launcher that runs the server in its own slice/unit, prove it by starting one and reading its socket, and only then schedule any run. This now blocks **every tier**, because the judge is an Internal API dispatch (§10 row k, §22).
- The Pi-runtime `commandcode/*` models come from the `commandcode-provider` Pi extension rather than from `COMMAND_CODE_ENABLED` (that flag gates the separate server-local Command Code *runtime*). Disposable mode keeps the Pi agent dir shared for extension loading, so the judge route *should* be present without `--command-code-real` — but this is exactly the kind of claim that must be **asserted at preflight, not assumed** (§21).
...
benchmarks/04-voice-live-lab/
  ...
  candidate_models.json     { "gemini-3.8-live": {...}, "gemini-3.8-live-extended-thinking": {"thinking": ["low","medium","high"]},
                              ...
                              "judge": {"runtime": "pi", "provider": "commandcode", "selector": "commandcode/deepseek/deepseek-v4.1-flash", "thinking": "high"},
...
2. **Disposable-server launcher.** The documented recipe is refused on this host (§26.3) and no working launcher was established during the 2026-09-16 audit, because working around a production-protection guard is not the agent's call. If the agent cannot find an authorised way to run the server outside the production slice, that is a question for the operator — and it blocks every tier, since the judge is an Internal API dispatch.
