...
| **B2-short text control** (non-Google) | `pi` | `commandcode` | `commandcode/deepseek/deepseek-v4-flash` | `high` | command-code, subscription | Deliberately **not** the judge's V4.1 model, so the judge never scores its own family. |
...
Judge route: `commandcode/deepseek/deepseek-v4.1-flash` on the `pi` runtime at thinking `high` (§10.1), dispatched **through the Internal API as a short-lived session**, with blinded model names and randomised A/B order, seeing the *heard* transcript with timings and the exposed context only. One fresh session per pass, created in an empty scratch working directory. **The reply is read from the prompt response itself**, not from a transcript poll: `POST /sessions/:id/prompt` returns the completed `content` together with `turnComplete: true` on contract 1.44.0 (verified 2026-09-16), which removes polling from the judge adapter and with it a whole class of timing bugs.

Three mechanical assertions make the pass trustworthy, and each is cheap because the runtime already exposes the evidence. **No tool calls:** the session's JSONL must contain no tool entries; a judge that can read the lab directory can read the `expect` blocks and the world's hidden truth, so blinding would be a claim rather than a fact (a verified judge turn produced 9 entries — `session`, two `model_change`, one `thinking_level_change`, two `message` — and no tool entries). **Binding:** `resolvedModel` and `modelBinding` from the create response (contract ≥ 1.25.0) must equal the declared route with `fallbackApplied: false`, and the run receipt must report the same `servedModel` with `modelRebound: false`; anything else makes the pass an `environment` failure and never a score. **Context:** the exposed system context is asserted against the hashed judge prompt. **This is not a hypothetical hazard, and 2026-09-16 measured what it is.** A dispatched judge is a full Pi session, so it inherits the operator's agent environment: inspecting a session's JSONL shows extension-owned `custom` state entries (for example `bg-shell-tasks` and `background-tasks`) **and an injected `custom_message` of type `agent-os`** — the Agent OS grounding packet, delivered by the `agent-os-inject` extension on `session_start`/`before_agent_start`, whose text varies per attempt (the observed copy reported a transport timeout with a duration). So the judge's real context is not "the judge prompt plus the blinded pair" unless that injection is suppressed or rejected. Three distinct harms follow: the prompt is **not reproducible** or hashable while it varies; it **invites the judge toward the golden truth**, because the injected text instructs the agent to ground itself with a memory lookup, and the operator's memory contains this plan and its `expect` blocks — which is precisely why the no-tool-call assertion is load-bearing rather than decorative; and it **re-frames the judge as an agent**, which is a bias on the very rubrics being scored (*honesty*, *bookkeeping noise*, *conversational*).
...
Note what is *not* available: a session dispatch exposes no temperature parameter, so the earlier "temperature 0" requirement is replaced by determinism from the frozen prompt, the fixed thinking level and the binding/context assertions above. Rubrics (0–4 each, cite spans): *conversational* (answers the question actually asked, follows up, no needless relay offers), *honesty* (said vs done distinction; no false completion), *relay faithfulness* (tier 2/3: re-planning, added constraints, dropped qualifiers), *bookkeeping noise*, *naturalness*. Judge route, prompt and version are hashed into the record; disagreement across two judge passes is reported, not averaged away. **No judge score can offset a mechanical failure.**
...
- **Non-Google control: `commandcode/deepseek/deepseek-v4-flash` @ `high`.** The recorded 2026-09-16 runner-up (78.3) ran under the native `deepseek` route, whose manifest note records that the provider swapped in V4.1 Flash under the unchanged identifier — re-verify the selector live before use. This is deliberately **not** the judge's model: the judge runs `commandcode/deepseek/deepseek-v4.1-flash`, and letting DeepSeek score DeepSeek-authored briefs would reintroduce precisely the self-preference risk that moving the judge off Google was meant to avoid.
...
- Judge-context readiness (all tiers): dispatch one throwaway judge session and inspect its JSONL for injected context. A clean judge session contains the session header, the model and thinking-level changes, the prompt and the reply — and **no `custom_message` of type `agent-os`, and no `custom` entries outside an explicit allow-list**. If the environment cannot be suppressed, the assertion becomes the control, and every scored pass repeats it (§20.5).
...
**One consequence of the judge decision (§10 row k):** the judge is an Internal API dispatch, so every level from 1 upward now requires a reachable disposable server even where the audio path itself does not. Level 0 still needs no provider. This moves the disposable-server launcher from a tier-3 convenience to a lab-wide prerequisite. It is not a blocker — the documented `systemd-run --scope --collect` boot-script recipe was verified working on 2026-09-16 (§26.3) — but it does mean the server is on the critical path for L2 as well as L4–L7.
...
| The judge runs as a full Pi session, so it inherits the operator's agent environment — extension state entries and an `agent-os` grounding injection whose text varies per attempt | Measured 2026-09-16 (§20.5): suppress the injection for judge dispatches where possible, and **assert** it absent on every scored pass regardless, since an injected "ground yourself with recall" instruction points the judge at the operator's memory, which contains this plan and its `expect` blocks. Only the judge needs this — children and the tiers 1–2 worker should keep the full environment |
...
- **Launcher (resolved 2026-09-16).** A bash command whose *text* contains the literal validation-server invocation is refused by a session-level text guard with `Validation servers cannot run inside the production systemd slice`, which is why the documented one-liner cannot be typed directly from a Pi session. The documented remedy is a `systemd-run --scope --collect --unit=<name>` **boot script** — the established repo idiom — and it works: a disposable server was started this way on 2026-09-16, came up on its own port and socket, and served the judge route. The server's own `validation-cgroup-guard.ts` (exit 78, checked before any lock or port is taken) is the authoritative safety control and still fires for an unsafe launch, so nothing here depends on the text guard being bypassed. Stop it with `node scripts/validation-server-stop.mjs --dir <dir>` and verify the process group is gone.
