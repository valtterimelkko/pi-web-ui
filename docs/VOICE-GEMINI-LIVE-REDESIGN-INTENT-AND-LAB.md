...
| **f** | **Simulated operator model: `zai/glm-5.3-flash` (thinking low). Judge model: Claude Sonnet 5 via OpenRouter, blind and pairwise. Both non-Google; both swappable in config.** | The candidate must never be its own operator, transcriber or judge (shared failure modes). Existing quota; outside the GLM peak window. |
...
**Model:** `zai/glm-5.3-flash`, thinking low, temperature 0.7, via the existing Pi Web UI Internal API *or* a direct zai call (direct is preferred: no session overhead). **Never the candidate's vendor.**
