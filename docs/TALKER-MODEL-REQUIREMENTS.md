...
> ## ⚠️ Status — corrected 2026-09-17: the role premise below is no longer current
>
> **Two of this brief's premises have been overtaken, and one of them twice.**
>
> 1. **“No tools, no reasoning burden.”** The
>    [renewed intent](./VOICE-MODE-INTENT.md) Part III deliberately frees the
>    talker to reason and to retrieve read-only material, replacing
>    *“answer only from the state snapshot”* with a **provenance rule**: every
>    statement must be auditable as *Reported*, *Derived* or *Mine*, with quotes
>    and completion claims rendered by the host rather than generated by the
>    model. A talker that refuses to reason is no longer the target.
> 2. **“A small, cheap, fast model is the hypothesis.”** The conversational seat
>    is decided: a **standard native-audio model**, with **no escalation
>    machinery of any kind** (owner decisions D3 and D6, 2026-09-17). Depth, when
>    it is needed, is the *worker's* job.
>
> What survives and remains useful: the latency budget (≤2 s to first token, p90),
> faithful relay as the hardest quality, rule-adherence under conversational
> pressure, speakable output, the screened-out failure modes, and the evaluation
> method. This file remains the historical record of how the original relay seat
> was chosen and benchmarked — the benchmark that selected the model currently in
> production.
>
> **Current brief:** [`VOICE-MODE-INTENT.md`](./VOICE-MODE-INTENT.md) §24 (standing
> model requirements) and §17 (provenance). **Map:**
