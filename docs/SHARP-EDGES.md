...
- **A watch's ledger and observer both survive restart.** A reloaded `active` watch is *rehydrated*: its conditions are re-resolved, the broker subscription re-established, subject/target pins re-acquired, the runtime observer re-attached, and it keeps recording without client action (watch-defect fix, 2026-09-10). Only a watch whose persisted conditions can no longer be resolved demotes to `detached` — past firings stay readable and re-registration is then required. A watch owns a distinct `watch:<watchId>` residency claim; replacement/deletion releases exactly that claim without affecting human UI or Internal API lease claims.
