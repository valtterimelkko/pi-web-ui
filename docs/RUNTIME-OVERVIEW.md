...
² Command Code catalogue is the CLI's advertised models minus a maintained GOAT premium exclusion list (denylist, fails open), regenerated by `npm run commandcode:refresh-models`; a model with empty `effortLevels` remains runnable without a `selector` (weekly refresh). Generic `thinkingLevel` and native `effort` are distinct.
...
maintained premium exclusion list) and fails open; effort selectors come from a
