...
- **Internal API run liveness and recovery (`1.14.0`)** — durable payload-free eligible-activity evidence, distinct idle/absolute watchdog causes, bounded late terminal observations, explicit cessation confidence, and retention/residency-aware session evidence. See [`ORCHESTRATED-RUN-LIVENESS-AND-RECOVERY.md`](./ORCHESTRATED-RUN-LIVENESS-AND-RECOVERY.md) and [`INTERNAL-API-CONTRACT.md`](./INTERNAL-API-CONTRACT.md).
