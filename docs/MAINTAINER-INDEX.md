...
- **Orchestrated run liveness and recovery intent** — rationale and open questions for attributable activity, idle-versus-absolute stalls, late terminal evidence, restart durability and honest partial-artifact survivability. This is design intent, not a shipped schema or implementation plan. See [`ORCHESTRATED-RUN-LIVENESS-AND-RECOVERY.md`](./ORCHESTRATED-RUN-LIVENESS-AND-RECOVERY.md).
- **Internal API dispatch truthfulness and session-identity integrity (`1.13.0`)** — idle Pi `follow_up` promotion, mode-aware busy pre-flight, approval responses that cannot return a false `200`, stalled-run watchdogs that release capacity, and fail-closed Pi session rehydration. See [`INTERNAL-API-CONTRACT.md`](./INTERNAL-API-CONTRACT.md) and the execution report [`plans/INTERNAL-API-DISPATCH-AND-IDENTITY-INTEGRITY-REPORT.md`](./plans/INTERNAL-API-DISPATCH-AND-IDENTITY-INTEGRITY-REPORT.md).
- **Internal API `1.13.0` truthful dispatch and Pi identity integrity** — requested and actual dispatch modes are distinct, accepted turns are watchdog-bounded, structured question answers require real resolution, and missing/mismatched Pi files fail closed; this builds on `1.12.0` source-owned retention/admission and `1.11.0` exact Claude profile binding. See [`INTERNAL-API-CONTRACT.md`](./INTERNAL-API-CONTRACT.md), [`INTERNAL-API.md`](./INTERNAL-API.md), and [`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md)
...
- [`ORCHESTRATED-RUN-LIVENESS-AND-RECOVERY.md`](./ORCHESTRATED-RUN-LIVENESS-AND-RECOVERY.md) — intent/rationale for future truthful run-liveness and recovery evidence; not current contract behavior
