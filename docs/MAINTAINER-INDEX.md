...
- **Voice Mode documentation spine (`2026-09-17`)** — one orientation file, [`VOICE-MODE-INDEX.md`](./VOICE-MODE-INDEX.md), now maps the ~25-document voice corpus: canonical intent and architecture first, then design inputs, lab execution records, plans and archive, with an explicit *“superseded and disproven — do not act on these”* list and a section stating plainly what the lab did and did not establish. The lab's sign-off, decision memo and published leaderboard were **annotated in place** (owner decision D1): the harness is real, the measured results are not. Two handover briefs that told a resuming agent to “read this first” while pinning contract 1.42.0 are demoted to history.
- **Native-voice target architecture decided (`2026-09-17`)** — [`VOICE-MODE-ARCHITECTURE-RECOMMENDATION-2026-09.md`](./VOICE-MODE-ARCHITECTURE-RECOMMENDATION-2026-09.md) records the owner's decisions D1–D7: keep the two lanes, replace the cascade with a native-audio conversational seat, move every load-bearing rule from the prompt into code and structured state, and defer ambient operation to a phone client. Its §2 is the **evidence audit** of the native-voice lab; its §7 is the gated build sequence. Not production approval.
- **Card identity, lanes, desktop pane and relay robustness (`2026-09-15…16`)** — see [`RECENT-CHANGES.md`](./RECENT-CHANGES.md) for the delta; the durable description lives in [`VOICE-MODE-INTENT.md`](./VOICE-MODE-INTENT.md) Parts II and IV.
...
- **Voice Mode (`2026-09-12…14`)** — the Drive Mode two-lane programme shipped end to end: server-side talker harness with a mechanical confirm-gated relay, transport binding over the browser WebSocket, client speech arbiter (anti-duet ladder), receipt ack, stop-talker, verbatim/summary/headlines reading levels, mobile socket durability, and voice observability. Canonical feature doc: [`VOICE-MODE-INTENT.md`](./VOICE-MODE-INTENT.md); observability records in [`OBSERVABILITY.md`](./OBSERVABILITY.md) §Voice Mode.
