...
- **Planned — Internal API dispatch truthfulness and session-identity integrity (`1.13.0`)** — evidence-backed plan to stop the API reporting success for silently discarded work: idle Pi `follow_up` promotion, busy pre-flight on every prompt mode, approval responses that can no longer return a false `200`, a stalled-run watchdog that releases admission capacity, and fail-closed Pi session identity on rehydration. Not yet executed; see [`plans/INTERNAL-API-DISPATCH-AND-IDENTITY-INTEGRITY-PLAN.md`](./plans/INTERNAL-API-DISPATCH-AND-IDENTITY-INTEGRITY-PLAN.md)
- **Internal API `1.12.0` source-owned retention/admission + `1.11.0` exact Claude profile binding** — Web UI/API/watch claims are independently owned; required durable/resident leases are atomic and API leases do not consume human pin slots; `/capacity` gates active turns using interactive reserve and measured memory headroom; exact Claude profile selectors remain separate from effective runtime models. See [`INTERNAL-API-CONTRACT.md`](./INTERNAL-API-CONTRACT.md), [`INTERNAL-API.md`](./INTERNAL-API.md), and [`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md)
