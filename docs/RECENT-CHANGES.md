...
- **Voice Mode documentation spine, and the native-voice lab (`2026-09-15…17`)**
  ...
  - **The lab (`L0`–`L8`).** A native-voice evaluation harness now exists: `scripts/voice-live-lab/**` (scheduler, frozen fixtures, paced PCM driver, reference playback, immutable attempt records with offline verification, baseline-cascade and native-live providers, tier harnesses, an adaptive operator instrument, and a 20-utterance fidelity corpus), with 19 test files in `server/tests/voice-live-lab/**` and packaging in the sibling `/root/agent-benchmarks/benchmarks/04-voice-live-lab/`. **Delivered equipment only — the scored runs were never performed** (the only committed manifest is a dry run with `realProviderCalls: 0`), and the published leaderboard figures trace to no data file.
  - **Evidence record corrected in place (D1).** The lab's decision memo, status ledger, implementation plan and the lab specification now carry an evidence-status banner pointing at the architecture doc's audit; the memo's completion verdict is marked **superseded**. Historical text is preserved unedited rather than erased.
  - **Confirmation-card identity (`1.44.0`).** The confirmation card carries and echoes an identity for the exact bytes it displays, so a confirmation can only release what was actually approved; stale cards refuse, and an “original wording” variant releases only where the current proposal advertised one.
  - **Lanes and the desktop pane (`2026-09-15…16`).** Up to three worker lanes in one tab with a lane strip, per-lane floor state and per-lane reading level; a desktop layout whose bottom pane is the *real* session view (shared virtualised list, no fork); switching a worker in place without losing capture, cards or focus.
  - **Relay robustness (`d27e75c`).** A relayed instruction now survives an idle or restarted worker: the earlier path-keyed lookup failed silently and the UI claimed success. From the same report, talker narration of worker bookkeeping was removed as noise.
  - **Routine injections stay out of the spoken turn.** Agent OS session-end capture injections are marked structurally at the emitter (`customType: 'agent-os-capture'`) and excluded from the spoken turn only — never by a text heuristic.

- **Voice Mode programme shipped (`2026-09-12…14`)**
  - The Drive Mode two-lane programme landed end to end. **Lane 1 — the talker:** a server-side conversational harness (`server/src/talker/*`) that holds the spoken conversation while a worker runs. Its send gate is **mechanical, not instructed**: the only code that can hand text to the worker runs solely when (a) a pending proposal holds the operator's verbatim utterance and (b) the operator's next utterance mechanically classifies as a confirmation — model output is never an input to the gate, and the relay text is always the operator's own words referenced by id, never model-composed. **Lane 2 — the work lane:** the ordinary worker session, untouched.
  - Transport binding: the browser's voice surface reaches the talker through `talker_turn` / `talker_turn_result` / `talker_digest` WebSocket messages (`H7`); every UI relay delivers (`ed3ea2f`). Delivery adapters degrade honestly per runtime — Pi mid-run `steer` via the existing path, Claude SDK steer/follow-up (non-SDK backends refuse rather than silently degrade), Antigravity follow-up queueing as a first-class outcome.
  - **Speech policy (client arbiter):** the anti-duet rule is a priority ladder — the operator speaking is never interrupted (playing audio ducks, never hard-stops), an unacknowledged utterance gets one fixed **receipt ack** ("Noted — still holding that."), the worker's completed answer speaks at the next natural gap, and talker chatter is dropped rather than queued. Capture is unconditional; only playback is scheduled.
  - **Reading levels (`P17`):** verbatim / summary / headlines, changeable mid-answer at a chunk boundary without repeating what was already heard; short turns (~≤400 chars) still speak verbatim under Summary; Headlines is the one line "Done: X. Needs you: Y." — a different extraction, not a shorter summary.
  - **Robustness:** mobile socket durability (`E1` — reconnect on resume/online/focus with a reset attempt budget, queued sends that flush in order after reconnect, failed voice sends never discard the dictation transcript), barge-in crash fix with client-side voice errors surfaced (`P13`), and the server event-loop stall root-caused and fixed (`R1`/`R2` — broker eviction counter 659,400 → 0).
  - **Observability (`P10`/`P11`/`P12`):** every voice turn emits one `voiceTurnId`-correlated record; releases, gate refusals, delivery outcomes, counters, and client speech decisions are reconstructable through the documented diagnostics path. Canonical feature doc: [`VOICE-MODE-INTENT.md`](./VOICE-MODE-INTENT.md); the records/queries live in [`OBSERVABILITY.md`](./OBSERVABILITY.md) §Voice Mode.

- **Lint ratchet headroom restored (`2026-09-14`)**
  - ~1,400 of the ~1,700 whole-tree ESLint warnings were `no-explicit-any` / `no-non-null-assertion` in **test and script files**, where mock stubs and `!` assertions are normal practice. `.eslintrc.json` now exempts test/script globs for those two rules; production code (`server/src`, `client/src`, `shared/src`, `packages/internal-api-mcp/src`) keeps strict enforcement. Actual warnings fell **1,700 → 306** and the whole-tree ceiling was re-baselined from 1,738 to **326** (306 + 20 genuine margin) — leaving the old ceiling would have made the ratchet a dead gate that could never fire. Policy and re-baseline rules: [`../tests/README.md`](../tests/README.md) §Lint ratchet policy; gate: `npm run lint:ratchet`.

- **Secrets migrated out of the repository (`2026-09-13`)**
  - Every live secret now sits outside the repo tree: `/root/.pi-web-ui/secrets.env` (mode 600), loaded by a systemd drop-in **after** `.env.production`. The formerly load-bearing repo `.env` moved to `/root/.pi-web-ui/env.dev` (mode 600) and is no longer load-bearing; `.env.production` now holds **zero** secret variables; `.env.example` values were verified to be placeholders distinct from live values. Nothing live was ever committed. Operator-owned follow-up (recorded, not done): rotation of the values that sat world-readable before the migration. Layout: [`../DEPLOYMENT.md`](../DEPLOYMENT.md) §Secrets layout; posture: [`../SECURITY.md`](../SECURITY.md) §Secrets hygiene.
...
- **Session-discovery ergonomics (`1.30.0`, `2026-09-02`)**
  ...
  - Worked evidence: the past-30-days session retrospective (all launch surfaces, provenance + log paths, 325 artefacts) was generated from these exact endpoints; the generated file lives on the host under `~/.pi-web-ui/notes/` (not committed — it contains session metadata), see [`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md) evidence-ladder step 0.
...
- **Browser workspace and chat ergonomics**
  ...
  - Canonical docs: [`SESSION-METADATA.md`](./SESSION-METADATA.md), [`DRIVE-MODE.md`](./DRIVE-MODE.md), [`PROTOCOL.md`](./PROTOCOL.md), [`EVENT-PIPELINE.md`](./EVENT-PIPELINE.md), [`ANTIGRAVITY-INTEGRATION.md`](./ANTIGRAVITY-INTEGRATION.md), [`SHARP-EDGES.md`](./SHARP-EDGES.md), [`PI-CODEX-COMPACTION-SESSION-ID.md`](./archive/PI-CODEX-COMPACTION-SESSION-ID.md)
