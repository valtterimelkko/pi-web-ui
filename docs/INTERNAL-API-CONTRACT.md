...
- **1.47.0** (minor, additive only — four orchestration improvements from the 2026-09-25 Claude-orchestrator validation matrix, section C). No existing field, default or behaviour changes for any runtime or client that does not opt in.
  ...
  - **C3 — server-side `deadline` watch condition.** `{ "type": "deadline", "afterSeconds": N }` (integer 1..86400; `once:false` rejected) fires exactly once N seconds after registration as a normal firing with `eventType: "deadline"`; usable with `onFire` and by pure observers (`/watches/wait`). The absolute due time is persisted on the condition (`dueAt`, epoch ms) and survives restart: an overdue deadline fires on boot, marked `reconciled: true`. It never dispatches to or marks the subject busy. Capability: `features.watchDeadlineCondition`.
  - **C4 — `fireIfSettled` on watch registration.** Register option `fireIfSettled: true` (default `false`): when the subject is idle and its most recent run receipt is terminal, each completion-type condition (`event_type` `agent_end` or `goal_end` without `dataMatch`) records one immediate firing marked `reconciled: true` (and dispatches `onFire` as usual). The register response echoes `fireIfSettled: { requested, settled, reason?, lastRunId?, lastRunStatus?, firedConditionIds, skippedConditionIds? }` only when requested. Capability: `features.watchFireIfSettled`.
  - **Amendment 1 — per-session Agent OS capture opt-in (owner decision 2026-09-25).** `POST /sessions` and each `POST /sessions/batch` entry accept optional `agentOsCapture: "enabled" | "disabled"` (any other value → 400). It is stored on the registry entry (`agentOsCapture`), echoed in the create response / batch result item and on `GET /sessions/:id` (and `/info`), and exported to per-session runtime subprocesses as `PI_WEB_UI_AGENT_OS_CAPTURE` — each only when set; absent = unspecified. Pi Web UI only records and exports the value; the capture decision (enabled → deliver, disabled → skip, unspecified + `internal-api` origin → skip unless `AGENT_OS_CAPTURE_API_SESSIONS=1`, anything else or any lookup failure → deliver) belongs to Agent OS. Capability: `features.sessionAgentOsCapture`.
  - Additive wire fields: `WatchFiring.reconciled`, `WatchConditionState.dueAt`, `RunReceipt.finalText` / `finalTextTruncated`, register response `fireIfSettled`, create/detail `agentOsCapture`. Consumer guidance: feature-detect via `/capabilities`; clients that ignore unknown fields are unaffected. Rollback: reverting the server removes all four. Two persisted-state effects: receipts written by 1.47.0 that carry `finalText` fail the older store's key allow-list and are ignored at load (a warning is logged; `GET /runs/:id` then answers `RUN_NOT_FOUND`), and an active watch carrying a `deadline` condition stays active on the older server but its deadline never fires. Delete such watches (and accept the receipt loss, or strip the two fields) before a rollback if that matters.
...
- **1.45.0** (minor, additive fields + ONE deliberate behaviour change — the silent no-op fixes) — orchestrators can now rely on the API to fail loudly instead of reporting success for things that did not happen (the 23 Sep conductor loss). Five wire-visible changes; four are additive, one is a deliberate behaviour change recorded here per house policy:
  ...
  - **`PROMPT_NOT_EXECUTED` fail-fast (additive code).** A Pi prompt (synchronous or detached) whose turn never started — the extension input-hook swallow, the 23 Sep shape — now fails the run with `PROMPT_NOT_EXECUTED` within seconds (2 s grace for a late `agent_start`), riding the normal receipt-terminal fan-out (watches and `child_turn_ended` fire as for any failed run), instead of stalling to the 15-minute watchdog. Runs that saw a compaction event, an `agent_start`, or a session that is streaming are unaffected; steer/follow_up/queued-turn semantics are untouched (1.29.0/2cd7a411 pinned behaviour).
  - **Voice relay honesty (additive typed error).** `MultiSessionManager.submitPrompt()` (the M3 submission-shaped prompt) now throws a typed not-submitted error when the worker never starts a turn after the same grace, so the talker delivery reports `refused` (amber NOT-sent) instead of a green "Sent"; M3 resolution timing for genuinely started turns is unchanged (28c6d644 receipts still settle at submission).
  - **Pi control lazy-load + honest create echo (additive behaviour on existing codes).** `set_thinking_level` on a registered-but-unloaded Pi session now lazy-loads it (dispatch-path pattern) and succeeds, instead of `404 SESSION_NOT_FOUND "Pi session not loaded"`; genuinely unloadable sessions still fail loudly. `POST /sessions` for Pi now echoes the actual post-clamp `thinkingLevel` (previously the field was absent), or `thinkingLevel: null` plus a `thinkingLevelNote` when the model reported no active level for the requested effort.
  - **Session ownership awareness (additive fields + two additive codes).** Pi sessions expose a display-only `ownership` object on `GET /sessions/:id` and on the adopt responses (`status: unknown|unmanaged|owned|conflict|uncertain`, plus `reason`, `ownerPid`, `ownerMode`, `updatedAt`, `leaseState` when known; `unknown` when the extension does not publish, and unknown never gates). Prompt, goal and control actions on a Pi session whose lease is held by a LIVE foreign runtime now refuse **`409 SESSION_OWNED_BY_OTHER_RUNTIME`** before any run is created, carrying `ownerPid`, `ownerMode`, `reason` and a hint; liveness is proven by pid liveness AND the lease's recorded process-start identity (a recycled pid counts as dead; uncertain evidence fails closed). When the recorded owner is dead, the next prompt/goal/control automatically recovers the session (dispose→rehydrate, pins preserved, model binding re-applied) and proceeds; a session that remains fenced answers **`409 SESSION_FENCED`**. Adoption remains display-only (1.40.0 rule) — it reports `ownership` but never recovers or modifies leases.
  - Rollback for the additive items: reverting the server removes the new fields and codes; no stored state is involved.
