...
- **1.17.0** (minor, additive server-local Command Code runtime) — adds the feature-gated `commandcode` runtime to the authenticated Internal API only:
  ...
  - `/models` and `/capabilities` advertise model-specific native `effort`: Qwen exposes exactly `low`, `medium`, `xhigh` with default `medium`, while Muse exposes no adjustable effort; unknown or drifted capability evidence fails closed, and `effort` is never mapped to generic `thinkingLevel`;
  - Command Code session records, prompts, run receipts, session evidence, and control support preserve requested/accepted/default/effective effort separately; `set_effort` is idle-only and applies to the next turn, while provider-reported effective effort is recorded only when present;
