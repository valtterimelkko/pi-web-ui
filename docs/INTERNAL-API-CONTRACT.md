...
- **1.17.0** (minor, additive Command Code runtime) — adds the feature-gated `commandcode` runtime:
  ...
  - the pinned Command Code execution policy remains `1.19.0` and the exact Agent OS shadow pair; a newer installed CLI is reported as version drift and cannot broaden execution, while its freshly parsed catalogue may remain visible as evidence.
